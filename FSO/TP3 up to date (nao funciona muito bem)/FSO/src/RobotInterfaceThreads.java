...
//		this.segueparedeT = null;
		...
		this.robot.SetSensorTouch(RobotLego.S_2);
	...
	private void launchThreads(){
		...
		this.cEvitar.start();
		this.cSegue = new SegueParede(this.robot,this.acessoRobot,this.dist_ctrl);
		this.cSegue.start();
	...
	private void setDist_ctrl(String dist_ctrl){
		try{	
			this.dist_ctrl = Integer.parseInt(dist_ctrl);
		} catch (Exception e) {
			showMessages("Erro seguinte: " + e.getMessage());
		}
	}
...
//		this.cVaguear.myWait();
	...
	public RobotInterfaceThreads() {
		...
		d_ctrlTextfield.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				setDist_ctrl(d_ctrlTextfield.getText());
				showMessages("dist_ctrl -> " + dist_ctrl);
			}
		});
