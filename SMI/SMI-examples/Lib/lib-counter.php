...
function getCounterValueAndIncrement($counterId) {
  ...
  mysql_query($queryIncrement, $GLOBALS['ligacao']);
  
  $queryGet = "SELECT `counterValue` FROM `images-counters` WHERE `id`='$counterId'";
  $resultGet = mysql_query($queryGet, $GLOBALS['ligacao']);
  $counterData = mysql_fetch_array( $resultGet );
  mysql_free_result($resultGet);
...
function buildCounterImage($counterValue) {
  ...
  while ($i < $numDigits) {
    ...
    ImageCopyResized($new, $src, $nx, 0, $sx, 0, $cutx, $srcy, $cutx, $srcy);
    $i+=1;
  }
