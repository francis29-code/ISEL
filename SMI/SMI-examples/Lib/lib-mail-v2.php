...
function sendAuthEmail(
    ...
    $fromEmail,
    // The friendly name that corresponds to the 
    // name used in the authentication
    $fromName,
    // The to list
    $toList,
    // The cc list
...
function encodeHeaderEmailList($headerName, $emailList, $srcEncoding = 'UTF-8', $dstEncoding = 'ISO-8859-1') {
  ...
  foreach ($emailList as $email) {
    ...
    $headers .= $encodeStart . iconv($srcEncoding, $dstEncoding . "//IGNORE", $email['display']) . $encodeEnd . " <" . $email['e-mail'] . ">";
  }
