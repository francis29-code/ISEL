...
def espectro(Fs, sin):                          #função que permite gerar o espectro da sinusóide e/ou gravação de voz
    ...
    X = np.fft.fftshift(sinusoide_fft)          #é aplicado um shitf a essa transformada para corrigir o posicionamento
    f = np.linspace(-Fs/2, Fs/2, len(X))        #de -Fs/2 a Fs/2, todos os valores da janela de x, vão estar igualmente espaçados por len(X)
    plt.plot(f,X)
    plt.title('Espectro do Sinal')
    plt.xlabel('Frequencia')
    plt.ylabel('Amplitude')
    pylab.xlim([-4000,4000])                    #Zoom em x
    pylab.ylim([0,6e+7])                        #Zoom em y
