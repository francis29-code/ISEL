...
if __name__== "__main__":
    ...
    image_name = "data/" + name + extensao
    image_grey_name = "data/" + name + "_grey" + extensao
    image_out_name = "data/" + name + "_out" + extensao
    print "Tempos obitos:"

    # Leitura da imagem pretendida e conversão para tons de cinzento
    img = cv2.imread(image_name, cv2.CV_LOAD_IMAGE_GRAYSCALE)
    cv2.imwrite(image_grey_name,img)
    shape = img.shape
    ...
    print "Número de bits por símbolo: ", nBits_simb
    eficiencia = eficiencia(entropia, nBits_simb)
    print "Eficiência: ", eficiencia
    print " "

    print "-------------------------------------------------------------------"
    print "Tamanhos dos ficheiros:"
    size_original = path.getsize(image_grey_name)
    print "Tamanho da imagem original: ", size_original
    size_descodificado = path.getsize(image_out_name)
    print "Tamanho da imagem descodificada: ", size_descodificado
    print "Taxa de Compressão: ", (1.*size_original/size_descodificado)
